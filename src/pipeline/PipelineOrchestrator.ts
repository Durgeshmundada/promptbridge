...
import {
  adaptTemplate,
  generateOptimizedPromptTemplate,
  generateTemplate,
} from './layer1/templateGenerator';
...
export interface PipelineExecutorEvents {
  question: string;
...
const API_OPTIMIZED_BADGE = 'Optimized prompt generated via API';
const LOW_CONFIDENCE_FALLBACK_BADGE = 'Used closest low-confidence template fallback';
...
/**
 * Orchestrates the PromptBridge pipeline by wiring together the seven enrichment and execution layers.
 */
export class PipelineOrchestrator extends TypedEventEmitter<PipelineExecutorEvents> {
  ...
  private async runExecution(
    ...
    try {
      ...
      // Keep template ranking anchored to the current prompt. Rich same-session context is still
      // available later for template adaptation/generation and in the final enriched prompt, but
      // feeding prior prompts into the matcher can cause one template family to "stick" across runs.
      const templateSearchInput = input.rawInput;
      ...
      const contextInjectionSeed = sessionSeed || input.rawInput;
      ...
      if (!templateOverride) {
        if (topMatch.zone === 'PARTIAL') {
          try {
            ...
            console.warn(
              '[PromptBridge][TemplateGenerator] Template adaptation failed; using the closest partial match.',
              error,
            );
          ...
          try {
            ...
            console.warn(
              '[PromptBridge][TemplateGenerator] Reusable template generation failed; trying one-off API prompt optimization.',
              error,
            );

            try {
              selectedTemplate = await generateOptimizedPromptTemplate(
                safeTemplateSearchInput,
                intent.intent,
                input.targetModel,
                templateRetrievalContext,
              ...
              matchZone = 'GENERATE';
              matchBadge = API_OPTIMIZED_BADGE;
              isNewTemplate = false;
            } catch (optimizationError) {
              console.warn(
                '[PromptBridge][TemplateGenerator] One-off API prompt optimization failed; using the closest low-confidence template as a final fallback.',
                optimizationError,
              );
              selectedTemplate = fallbackTemplate;
              matchZone = 'GENERATE';
              matchBadge = LOW_CONFIDENCE_FALLBACK_BADGE;
              isNewTemplate = false;
            }
          }
        }
